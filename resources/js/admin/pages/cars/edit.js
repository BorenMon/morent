    ...
    FileUpload.prototype.init = function () {
        ...
        $('[data-plugin="dropzone"]').each(function () {
            ...
            dropzoneEl.on("success", function (file, response) {
                console.log(
                    "Custom function triggered, file uploaded:",
                    response
                );
...
async function urlToFilePondObject(file) {
    ...
    return {
        ...
        options: {
            type: "local",
            metadata: {
                object_name: file.image_path,
            },
        },
    };
}

const carImages = JSON.parse(
    $('meta[name="car-images"]').attr("content") || "[]"
);

// Convert each image object properly
Promise.all(carImages.map(urlToFilePondObject)).then((existingImages) => {
    const uploadQueue = [];

    const pond = FilePond.create(
        document.querySelector('input[name="car-images"]'),
        {
            allowMultiple: true,
            ...
            onaddfile: async (error, file) => {
                ...
                // If file is already uploaded or reverted, skip confirmation
                if (
                ...
                // If the queue contains only one file, prompt the confirmation
                        ...
                        .then(async (result) => {
                            if (result.isConfirmed) {
                                try {
                                    ...
                                    await fetch(
                                        `/admin/cars/${carId}/images/${encodedPath}`,
                                        {
                                            method: "DELETE",
                                            headers: {
                                                "X-CSRF-TOKEN": csrfToken,
    ...
    async function processUploadQueue() {
        ...
        const result = await sweetalert2.fire({
            title: `Are you sure you want to upload file(s)?`,
            text: "Once added, they will be stored in the car's gallery.",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3563E9",
            cancelButtonColor: "#d33",
        ...
        if (result.isConfirmed) {
            ...
            uploadQueue.forEach((file) => pond.removeFile(file));
        }

        uploadQueue.length = 0; // Clear the queue after processing
    }
});
